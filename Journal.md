...
### 12月30日
今天实现了学生端答案写入本地临时文件以用于灾备，至此，该项目的`Plans.md`中的任务已全部完成，后面还有期末考试的压力，可能不会再去实现其它功能，现在要准备开始写文档提交了。

从设计方案，到开始实现，达到每个里程碑，到今天基本完成，这一过程属实不易，完成的一刻我们心里也有说不出的感受。总之，我们还是收获了很多的，详见文档吧！

### 12月31日
找了同学进行内测，发现三个用户体验上的小问题，进行了修复：
1. 启动/停止服务没有反馈 -> 主界面底部增加服务器状态显示；
2. 没有点击新建考试时，也会有添加题目等按键 -> 禁用按键，点击新建考试后启用；
3. 新建的考试需要退出重进后才能显示 -> 保存考试后自动更新。

发现一些其他问题，例如考生迟到则无法进入，不过似乎也合理（不是
以后有机会再解决吧~

下午又增加了异常处理，在教师端创建考试时，如果漏填了信息，原本是不能保存且没有报错，当使用者没注意到时会很困扰。现在增加了错误信息提示，并提示时间、分数等必须是数字。
导出为CSV时，中文会出现乱码。于是修改了导出为CSV的方法，弃用FileWriter.